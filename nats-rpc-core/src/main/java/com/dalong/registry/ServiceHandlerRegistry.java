...
public class ServiceHandlerRegistry {
    ...
    public <T extends BaseMessage> void registerWithSubTypes(ServiceHandler<T> handler, ObjectMapper objectMapper) {
        ...
        handler.registerSubTypes(objectMapper);
        messageTypes.put(messageType.getSimpleName(), messageType);
        messageTypes.put(messageType.getName(), messageType);
        log.info("Registered message handler for type: {}", messageType.getSimpleName());
    }
     ...
     * @param <T>
     */
    public <T extends BaseMessage> void register(ServiceHandler<T> handler) {
        Class<T> messageType = handler.getMessageType();
        handlers.put(messageType, handler);
        messageTypes.put(messageType.getSimpleName(), messageType);
        messageTypes.put(messageType.getName(), messageType);
    ...
    public Map<Class<? extends BaseMessage>, ServiceHandler<? extends BaseMessage>> getHandlers() {
        return handlers;
    }
    ...
    public <T extends BaseMessage> ServiceHandler<T> getHandler(Class<T> messageType) {
        return (ServiceHandler<T>) handlers.get(messageType);
    }

    public Method getMessageActionHandlerMethod(BaseMessage message) {
        ServiceHandler handler = getHandler(message.getClass());
        if (handler != null) {
